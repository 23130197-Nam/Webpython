const API_ENDPOINTS = {
    ...
    applyVoucher: '/api/promotion/apply/'
};
...
function renderAddresses(addresses) {
    console.log("renderAddresses đã được gọi")
    const modalBody = document.getElementById('modal-address-list');
    modalBody.innerHTML = addresses.map(addr => `
        <label class="address-option-item" style="display: block; padding: 15px; border: 1px solid #eee; border-radius: 8px; margin-bottom: 10px; cursor: pointer;">
            <div style="display: flex; gap: 10px;">
                <input type="radio" name="addr-choice" value="${addr.id}" ${addr.id === checkoutState.selectedAddressId ? 'checked' : ''}>
                <div style="flex: 1;">
                    <div style="font-weight: bold;">${addr.receiver_name} | ${addr.receiver_phone}</div>
                    <p style="margin: 5px 0 0; font-size: 13px; color: #666;">${addr.full_address}</p>
                    ${addr.is_default ? '<span style="font-size: 11px; color: #2d88ff; background: #e7f3ff; padding: 2px 6px; border-radius: 4px;">Mặc định</span>' : ''}
                </div>
            </div>
        </label>
    `).join('');
...
function renderVouchers(vouchers) {
    ...
    container.innerHTML = vouchers.map(v => `
        <div class="voucher-card" onclick="applyVoucher('${v.code}', ${v.discount_value})" id="v-${v.code}" 
             style="border: 1px solid #ddd; padding: 12px; margin-bottom: 10px; border-radius: 8px; cursor: pointer; position: relative;">
            <div style="font-weight: bold; color: #e44d26;">${v.code}</div>
            <div style="font-size: 12px; color: #555;">${v.description}</div>
        </div>
    `).join('');
}
...
function applyVoucher(code, val) {
    console.log("applyVoucher đã được gọi")
    document.querySelectorAll('.voucher-card').forEach(el => el.style.borderColor = '#ddd');
    checkoutState.selectedVoucherCode = code;
    checkoutState.discount = val;
...
function updateSummaryDisplay() {
    ...
    checkoutState.total = Math.max(0, checkoutState.subtotal - checkoutState.discount);
    
    document.getElementById('summary-subtotal').innerText = formatVND(checkoutState.subtotal);
    document.getElementById('summary-discount').innerText = `-${formatVND(checkoutState.discount)}`;
    document.getElementById('summary-total').innerText = formatVND(checkoutState.total);
}
